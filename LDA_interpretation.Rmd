...
Assumption: covariates have a multiplicative effect on the __Survival time__. E.g.: Survival time for smokers is an accelerated version of the survival time for non-smokers. We model the survival time directly with the log-trafo:
...
1. estimate $\beta$s via Cox  
1. estimate non-parametrically baseline hazards $\lambda_0(t)$ e.g. via Breslow   
1. ^^^calculate for each t $\lambda(t) = \lambda_0(t) exp(x_i'\beta)$  
1. cumulate the $\lambda(t)$ to the cumulative Hazards $\Lambda_{t} = \sum_{i = 1}^t \lambda_i$  
...
* log rank score test: tests for significant differencies in the survival curves for the two   subpopulations seperated by the variable of interest (here: treatment). This means that the probability of an event occurring at any time point is the same for each subpopulation. H0: they do not differ -> p > 0.05: H0 cannot be rejected -> no significant effect of treatment. \textcolor{red}{WHAT HAPPENS WITH MORE COVARIATES? E.G.: one significant, the other not}
...
What can we do? Exclude variable or model time varying effect // non-linearly e.g. using splines
