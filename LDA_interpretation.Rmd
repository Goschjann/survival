...
* __non-parametric__ $\lambda_0(t)$ via Breslow
...
<!--
$$
\lambda ( t|X_{it} ) = P( y_{it} = 1 | X_{it})  = \frac{ exp(\beta_{0t} + X_{it}^T\beta)}{1 + exp(\beta_{0t} + X^T_{it}\beta)}
$$
-->
and thus the baseline hazard (all other coavariates than the time dummy-variable of interest):

<!--
$$
\lambda_0 (t| X_{it} ) = P( y_{it} = 1 | X_{it} = 0) = \frac{ exp(\beta_{0t}) }{1 + exp(\beta_{0t} ) }
$$
-->
Thus, we yield 52 time coeffients next to the other coefficients:
...
<!--
$$
\text{CIC}_j(t_f) = P(T <= f | C = j) = \sum_{l = 1}^f \hat S(t_{l}) \times \hat \lambda_j(t_l) \text{ Aalen-Johannsen Estimator of cumulative incidence}
$$
-->

Latex does not work for some reason

Kaplan Meier would use event dependent $\hat S_c(t_{l-1})$ instead of overall $\hat S(t_{l-1})$
